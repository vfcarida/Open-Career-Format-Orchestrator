...
export class RedisApprovalStore implements IApprovalStore {
  ...
  generateToken(toolName: string, payload: unknown, metadata?: Record<string, unknown>, requesterIdentity?: string, ttlMs = 15 * 60 * 1000): string {
    ...
    // Store with TTL
    this.redis.setex(`${this.prefix}pending:${token}`, Math.floor(ttlMs / 1000), JSON.stringify(record));
  ...
  getPendingApprovals(): PendingApproval[] {
    ...
    console.warn('[RedisApprovalStore] getPendingApprovals requires a secondary index (not fully implemented in Redis adapter)');
    return [];
  }
  ...
  getAuditLogs(_limit = 100): AuditLog[] {
    ...
    console.warn('[RedisApprovalStore] getAuditLogs should query a persistent sink, not Redis.');
    return [];
  }

  async validateAndConsume(token: string, toolName: string, payload: unknown, actorIdentity?: string): Promise<boolean> {
  ...
  private logAudit(action: string, toolName: string, payloadHash: string, metadata?: Record<string, unknown>, actorIdentity?: string) {
    const log: AuditLog = {
      timestamp: Date.now(),
      action,
      toolName,
    ...
    this.redis.xadd(`${this.prefix}audit_stream`, '*', 'event', JSON.stringify(log));
  }
